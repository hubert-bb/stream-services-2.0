...
public class TransactionUnitOfWorkExecutor extends UnitOfWorkExecutor<TransactionTask> {
    ...
    public Flux<UnitOfWork<TransactionTask>> prepareUnitOfWork(List<SchemasTransactionPost> items) {
        ...
        if (((TransactionWorkerConfigurationProperties) streamWorkerConfiguration).isGroupPerArrangementId()) {
            Map<String, List<SchemasTransactionPost>> transactionsGroupedByArrangement = items.stream()
                .collect(Collectors.groupingBy(SchemasTransactionPost::getArrangementId));
    ...
    public Flux<UnitOfWork<TransactionTask>> prepareUnitOfWork(Flux<SchemasTransactionPost> items) {
